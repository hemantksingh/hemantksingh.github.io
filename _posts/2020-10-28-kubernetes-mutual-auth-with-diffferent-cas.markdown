...
You can have the client and the server certificates issued by the same CA or as shown below by separate CAs.

![mutual-auth.png](../assets/mutual-auth.png "Mutual authentication")

## Setting up mutual authentication

In order to configure mutual authentication for a host in a kubernetes cluster, we are going to assume you already have a kubernetes cluster up and running e.g. by using something like [Azure Kubernetes Service (AKS)](https://docs.microsoft.com/en-gb/azure/aks/kubernetes-walkthrough) and an [nginx ingress controller deployed](https://kubernetes.github.io/ingress-nginx/deploy/) to it. The ingress controller deployment on AKS configures nginx as a service that can be accessed outside the cluster via an `EXTERNAL_IP`. We are going to use wildcard DNS service [nip.io](https://nip.io/) to give this `EXTERNAL_IP` a domain name like `EXTERNAL_IP.nip.io` and configure server TLS for this domain.
